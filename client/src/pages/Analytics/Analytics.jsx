...
import { 
  ...
  Shield, CheckCircle, Users, Target, Loader2
} from 'lucide-react';
...
import { useTrends, useDepartmentMetrics, usePredictiveAnalysis } from '../../hooks/useAnalytics';
...
export default function Analytics({ user }) {
  ...
  const { data: departmentData, isLoading: deptLoading } = useDepartmentMetrics({
    groupId: user.group_id,
    teamId: user.team_id
  });
  
  const { data: predictiveData, isLoading: predictiveLoading } = usePredictiveAnalysis({
    enabled: user?.role === 'group_admin' || user?.role === 'super_admin',
    groupId: user.group_id
  });

  // Mock data for indicators (fallback if API not ready)
  const indicatorTrends = trendsData || [
  ...
  const departmentMetrics = departmentData || [
  ...
  const isLoading = trendsLoading || deptLoading || predictiveLoading;
    ...
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <>
          ...
          {(user?.role === 'group_admin' || user?.role === 'super_admin') && predictiveData && (
                  ...
                  <div className="p-4 rounded-lg bg-white dark:bg-slate-800 border">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <div className={`w-3 h-3 rounded-full animate-pulse ${
                        predictiveData.riskLevel === 'high' ? 'bg-red-500' :
                        predictiveData.riskLevel === 'medium' ? 'bg-amber-500' : 'bg-green-500'
                      }`} />
                      <p className="text-xl font-bold capitalize">{predictiveData.riskLevel || 'Moderate'}</p>
                    </div>
                    <p className="text-xs text-muted-foreground mt-1">{predictiveData.probability}% probability increase</p>
                  </div>
                  <div className="p-4 rounded-lg bg-white dark:bg-slate-800 border">
                    <p className="text-sm font-medium text-muted-foreground">Top Risk Area</p>
                    <p className="text-xl font-bold mt-1">{predictiveData.topRiskArea || 'Warehouse'}</p>
                    <p className="text-xs text-muted-foreground mt-1">{predictiveData.topRiskCause || 'Forklift operations'}</p>
                  </div>
                  <div className="p-4 rounded-lg bg-white dark:bg-slate-800 border">
                    <p className="text-sm font-medium text-muted-foreground">Recommendation</p>
                    <p className="text-xl font-bold mt-1">{predictiveData.recommendation || 'Immediate Review'}</p>
                    <p className="text-xs text-muted-foreground mt-1">Within {predictiveData.timeframe || '7'} days</p>
                  </div>
                </div>
                <div className="mt-4 p-4 rounded-lg bg-white dark:bg-slate-800 border">
                  ...
                  <p className="text-sm">
                    {predictiveData.summary || 
                      'Predictive analysis indicates a 45% increased risk of forklift-related incidents in Warehouse Zone A based on recent near-misses, delayed maintenance, and increased traffic patterns. Recommended actions include additional safety training and equipment inspections.'}
                  </p>
                </div>
              </CardContent>
            </Card>
